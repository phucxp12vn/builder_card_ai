...
const Slider = (props: {
  ...
  const { activeItem, positions, children, setActiveItem, initItemHeight } = props;
  const [ref, dimensions] = useBoundingRect();
  const height = dimensions?.height ?? 0;

  useLayoutEffect(() => initItemHeight(Math.round(height)), [height, initItemHeight]);
  ...
  const handleIncrementClick = () => {
    activeItem < positions.length - 1 && setActiveItem((prev: number) => prev + 1);
  };

  return (
    <>
      <HStack height={'100%'}>
        <Flex h={'100%'} direction="column" mx="auto" justifyContent="space-around">
          <IconButton
